...
%         T_X{count} = double(squeeze(data.X3d(i, chans, :, :)));
...
% % (trials, spatial features, spectral/temporal features)
% X_s = double(squeeze(data.X3d(s, chans, :, :)));
...
lbase = MT_logistic(size(T_X2d{1},1), 'dim_reduce',0, 'n_its',n_its);
logreg = MT_logistic_test(size(T_X2d{1},1), 'dim_reduce', 0, 'n_its', n_its);
...
fprintf('Logreg test version prior accuracy: %.2f\n', acc*100);
acc = mean(lbase.prior_predict(X2d_s) == y_s);
...
out = lbase.fit_new_task(X2d_s, y_s, 'ml', 1);
acc = mean(out.predict(X2d_s) == y_s);
fprintf('new task training accuracy (logreg test): %.2f\n', acc*100);
